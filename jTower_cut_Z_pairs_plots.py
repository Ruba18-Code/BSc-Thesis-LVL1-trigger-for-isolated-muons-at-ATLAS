...
res=muon_isolation_all_events(MuonTree_ZeroBias,ZeroBias_eta,ZeroBias_phi,dr_min,dr_max,[nmin,nmax],100)
data1=ak.flatten(res)

#Compute the isolation and prepare it for plotting Z mu mu
res=muon_isolation_all_events(MuonTree_Zmumu,Zmumu_eta,Zmumu_phi,dr_min,dr_max,[nmin,nmax],100)
...
#Plot the data
coolplot([data1,data2],np.linspace(0,30000,40),colors,labels, "Isolation (MeV)","Counts",
...
coolplot([ratio1,ratio2],np.linspace(0,1.1,40),colors,labels, "Isolation/et ratio","Counts",
...
bins=np.linspace(0,2,3*int(np.sqrt(nmax-nmin)))
dr_min=[0.0,0.0,0.3101]
dr_max=[0.4,0.5,0.5820]
